...
# repo (issue #21):
#   1. GF180_PDK_PATH (already a full variant dir, e.g. .../gf180mcuC)
#   2. PDK_ROOT + PDK (default variant: gf180mcuC)
...
#
# Sets on success:
#   GF180_VARIANT_DIR   -- .../gf180mcuC (or whichever variant)
...
_pdk_env_variant="${PDK:-gf180mcuC}"
_pdk_env_root=""
...
if [[ -n "${GF180_PDK_PATH:-}" ]]; then
  ...
  if [[ -z "$_pdk_env_root" ]]; then
    ...
    echo "pdk_env.sh: set PDK_ROOT (parent of gf180mcuC/) and PDK=gf180mcuC, or GF180_PDK_PATH=<.../gf180mcuC>, and re-run." >&2
