...
| 11 — Power delivery (structural) | unmet `no_evidence` | Uncited because the evidence does not exist yet — no `klt erc` supply spec or report in this repo. That is exactly the companion issue #124's scope; this row turns gradeable when its committed `klt erc` supply run lands, under a released `klt` that grades item 11 (or via the vendored checklist in the meantime, which renders the row as visible-unmet rather than no row at all). |
